...
class Home():
	...
	desc = 'Teleport player back to their spawn'
	...
	async def runCheer(self, user, amount):
		...
		with MCRcon(RCON_IP, PASSW, int(RCON_PORT)) as mcr:
			...
			resp = mcr.command('/spawn ' + str(HOST_USER))

			# Minecraft command to post notification text in the game
			resp = mcr.command('/tellraw @a [{\"text\":\"' + user + ': Teleported ' + str(HOST_USER) + ' to spawn\",\"color\":\"green\"}]')
			mcr.disconnect()
